...
test('GitHub workflows pass dynamic agent profile variables to alex ci', () => {
  for (const workflowPath of workflowPaths) {
    ...
    assert.equal(profileStep.env.DEFAULT_ALEX_PRESET, "${{ vars.ALEX_PRESET || '' }}");
    assert.match(profileStep.with.script, /readOption\('agents'\)/);
    assert.match(profileStep.with.script, /readOption\('disable-agents'\)/);
    assert.match(profileStep.with.script, /readOption\('preset'\)/);
    assert.match(profileStep.with.script, /core\.setOutput\('preset'/);
    assert.match(profileStep.with.script, /Keep in sync with src\/agents\/presets\.ts/);
    assert.match(profileStep.with.script, /core\.info\(`A\.L\.E\.X preset:/);
    ...
    assert.equal(runStep.env.ALEX_PRESET, "${{ steps.agent-profile.outputs.preset }}");
    ...
    assert.match(runStep.run, /ALEX_ARGS=\(\)/);
    assert.match(runStep.run, /--include-codebase-context/);
    assert.match(runStep.run, /--unsafe-disable-codebase-limits/);
    assert.match(runStep.run, /ALEX_ARGS\+=\(--preset "\$ALEX_PRESET"\)/);
    assert.match(runStep.run, /ALEX_ARGS\+=\(--agents "\$ALEX_AGENTS"\)/);
    assert.match(runStep.run, /"\$\{ALEX_ARGS\[@\]\}"/);
